...
def crop_phases(input_dir, output_dir, n_phases=25, cube_size=480, prefix=""):
    ...
    Args:
        ...
        prefix     : filename prefix, e.g. "r_" for registered files

    Also writes crop_info.json to output_dir for coordinate conversion.
    """
    os.makedirs(output_dir, exist_ok=True)
    print(f"[crop_phases] {input_dir} -> {output_dir}  cube={cube_size}  prefix='{prefix}'")

    ref_img        = nib.load(os.path.join(input_dir, f"{prefix}phase1.nii.gz"))
    ...
    for p in range(1, n_phases + 1):
        in_path  = os.path.join(input_dir,  f"{prefix}phase{p}.nii.gz")
        out_path = os.path.join(output_dir, f"{prefix}phase{p}.nii.gz")
    ...
    b0_in  = os.path.join(input_dir,  f"{prefix}b0.nii.gz")
    b0_out = os.path.join(output_dir, f"{prefix}b0.nii.gz")
